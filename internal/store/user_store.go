...
func (s *PostgresUserStore) CreateUser(user *User) error {
	query := `INSERT INTO users (username, email, password_hash, bio, created_at, updated_at)
			  VALUES ($1, $2, $3, $4, $5, $6)
			  RETURNING id, created_at, updated_at`
	err := s.db.QueryRow(query, user.Username, user.Email, user.PasswordHash.hash, user.Bio, time.Now(), time.Now()).Scan(&user.ID, &user.CreatedAt, &user.UpdatedAt)
	if err != nil {
		return err
	}
	return nil
...
func (s *PostgresUserStore) UpdateUser(user *User) error {
	...
	result, err := s.db.Exec(query, user.Username, user.Email, user.Bio, time.Now(), user.ID)
	if err != nil {
		return err
	}
